...
/**
 * Manager OAuth. It is used to manage the OAuth request.
 * 
 * @author Joan Fuentes
 * 
 */
public class OAuthGlobalManager {
	...
	public static OAuthGlobalManager newInstance(Context context,
			...
			String authorizationRequestUrl, String tokenServerUrl,
			final String redirectUri, List<String> scopes) {
		...
		SharedPreferencesCredentialStore credentialStore = new SharedPreferencesCredentialStore(
				context, Application.CREDENTIALS_STORE_PREF_FILE, JSON_FACTORY);
		...
		AuthorizationFlow.Builder flowBuilder = new AuthorizationFlow.Builder(
				...
				JSON_FACTORY, new GenericUrl(tokenServerUrl), client,
				client.getClientId(), authorizationRequestUrl)
				.setScopes(scopes).setCredentialStore(credentialStore);

		AuthorizationFlow flow = flowBuilder.build();
		// setup authorization UI controller
		AuthorizationDialogController controller = new DialogFragmentController(
				fragmentManager) {
		...
		return new OAuthGlobalManager(flow, controller);
	}

	protected final OAuthManager manager;

	private OAuthGlobalManager(AuthorizationFlow flow,
	...
	public OAuthFuture<Boolean> deleteCredential(String userId,
			OAuthCallback<Boolean> callback) {
		return deleteCredential(userId, callback, null);
	}

	public OAuthFuture<Boolean> deleteCredential(String userId,
			OAuthCallback<Boolean> callback, Handler handler) {
